...
async def get_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with open(filename, 'wb') as f:
        await new_file.download_to_memory(out=io.FileIO(fd, mode='w'))
    
    with requests.Session() as session:
    
        with session.post(os.path.join(os.environ['ADDRESS'], 'recognize'), 
                               files={'upload': open(filename, 'rb'), }) as resp:
            text = resp.json()
            print(text['result'])
            with sqlite3.connect('core.db') as conn:
                cr = conn.cursor()
                cr.execute('''INSERT INTO books (raw_str, user_id)
                                VALUES (:raw_str, 1)
                           ''', {"raw_str": text['result']})
